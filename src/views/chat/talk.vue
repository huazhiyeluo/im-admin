<template>
    <div>
        <div class="chat-messages" id="chatMessages" ref="chatMessages">
            <div v-for="item in state.historyContentList[state.rkey]"
                :class="{ 'message user-message': item.FromId === state.selftUserInfo.Uid, 'message other-message': item.FromId !== state.selftUserInfo.Uid }">
                <van-image width="50" height="50" round :src="item.Avatar" />
                <div class="message-content">
                    <p v-if="item.MsgMedia == 1">{{ getContent(item)}}</p>
                    <p v-if="item.MsgMedia == 2">
                        <van-image width="100" height="100" :src="getContent(item)" @click="previewPic(getContent(item))" />
                    </p>
                    ...
                    <p v-if="item.MsgMedia == 6">
                        <van-image width="40" height="40" round :src="getContent(item)" @click="previewPic(getContent(item))" />
                    </p>
        ...
        <div class="el-footer">
            <div class="input-container-inner">
                ...
                <span :size="30" @click="showPlugin" v-show="state.isInputEmpty" ref="showPluginRef"><van-icon name="add-o" :size="30" /></span>
            ...
            <div class="chat-smile" v-if="state.isShowSmile" ref="smileRef">
                <van-image v-for="item in state.emojs" width="30" height="30" :src="item" @click="sendEmojsMessage(item)" />
            </div>
            <div class="chat-plugins" v-if="state.isShowPlugin" ref="pluginRef">
                ...
                <van-uploader accept=".pdf,.doc,.docx,.zip,.rar,.7z,.csv"
                    :after-read="handleSuccessFile">
                    <van-icon name="description-o" class="icon" />
                </van-uploader>
                <van-uploader accept="audio/*"
                    :after-read="handleSuccessAudio">
                    <van-icon name="service-o" class="icon" />
                </van-uploader>
                <van-uploader accept="video/*"
                    :after-read="handleSuccessVideo">
