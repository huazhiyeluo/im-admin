        ...
        <div v-for="(item,key) in state.applys" v-if="state.applys.length > 0 ">
            <div class="contact-item" v-if="item.Type == 1 && item.FromId == state.selftUserInfo.Uid">
                ...
                <div class="user-operate">
                    <div class="operate" v-if="item.Status == 0"><div class="operate-right-30">等待验证</div></div>
                    <div class="operate" v-if="item.Status == 1"><div class="operate-right">已被同意</div></div>
                    <div class="operate" v-if="item.Status == 2"><div class="operate-right">已被拒绝</div></div>
                </div>
            </div>
            <div class="contact-item" v-if="item.Type == 1 && item.ToId == state.selftUserInfo.Uid">
                ...
                <div class="user-operate">
                    <div class="operate" v-if="item.Status == 0"><van-button type="success" size="small" @click="goOperate(key,item.Id ,1)">同意</van-button><van-button size="small" @click="goOperate(key,item.Id,2)">拒绝</van-button></div>
                    <div class="operate" v-if="item.Status == 1"><div class="operate-right">已同意</div></div>
                    <div class="operate" v-if="item.Status == 2"><div class="operate-right">已拒绝</div></div>
                </div>
            </div>
...
const loadUserManage = (data: any) => {
    ...
    if (res.apply){
        const isObjectInArray = state.applys.some(obj => obj.Id === res.apply.Id);
        if(isObjectInArray){
        for (const key in state.applys){
            if (state.applys[key].Id == res.apply.Id){
                state.applys[key] = res.apply
                break;
            }
        }
        }else{
